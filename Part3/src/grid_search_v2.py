				...
				if append:
					for existing in all_results["lsh"]:
						if (int(existing["N"]) == N and 
							...
							float(existing["w"]) == param_set["w"]):
							already_exists = True
							break
				
				if already_exists:
					print(f"\n[{run_count}] LSH: N={N}, k={param_set['k']}, L={param_set['L']}, w={param_set['w']} - SKIPPING (already exists)")
					continue
				
				param_str = f"N{N}_k{param_set['k']}_L{param_set['L']}_w{int(param_set['w'])}"
				output_txt = os.path.join(output_dir, f"lsh_{param_str}.txt")
				
				print(f"\n[{run_count}] LSH: N={N}, k={param_set['k']}, L={param_set['L']}, w={param_set['w']}")
	...
	if "hypercube" in methods_to_run:
		...
		for N in N_vals:
			for param_set in get_hypercube_grid():
				...
				if append:
					for existing in all_results["hypercube"]:
						if (int(existing["N"]) == N and 
							...
							float(existing["w"]) == param_set["w"] and
							int(existing["M"]) == param_set["M"] and
							int(existing["probes"]) == param_set["probes"]):
							already_exists = True
							break
				
				if already_exists:
					print(f"\n[{run_count}] Hypercube: N={N}, kproj={param_set['kproj']}, w={param_set['w']}, M={param_set['M']}, probes={param_set['probes']} - SKIPPING (already exists)")
					continue
				...
				print(f"\n[{run_count}] Hypercube: N={N}, kproj={param_set['kproj']}, w={param_set['w']}, M={param_set['M']}, probes={param_set['probes']}")
			...
			for param_set in get_ivfflat_grid():
				...
				if append:
					for existing in all_results["ivfflat"]:
						if (int(existing["N"]) == N and 
							int(existing["seed"]) == seed_val and
							int(existing["kclusters"]) == param_set["kclusters"] and
							int(existing["nprobe"]) == param_set["nprobe"]):
							already_exists = True
							break
				
				if already_exists:
					print(f"\n[{run_count}] IVF-Flat: N={N}, kclusters={param_set['kclusters']}, nprobe={param_set['nprobe']} - SKIPPING (already exists)")
					continue
				...
				print(f"\n[{run_count}] IVF-Flat: N={N}, kclusters={param_set['kclusters']}, nprobe={param_set['nprobe']}")
	...
	if "ivfpq" in methods_to_run:
		...
		for N in N_vals:
			for param_set in get_ivfpq_grid():
				...
				if append:
					for existing in all_results["ivfpq"]:
						if (int(existing["N"]) == N and 
							int(existing["seed"]) == seed_val and
							int(existing["kclusters"]) == param_set["kclusters"] and
							int(existing["nprobe"]) == param_set["nprobe"] and
							int(existing["ivfpq_M"]) == param_set["ivfpq_M"]):
							already_exists = True
							break
				
				if already_exists:
					print(f"\n[{run_count}] IVF-PQ: N={N}, kclusters={param_set['kclusters']}, nprobe={param_set['nprobe']}, M={param_set['ivfpq_M']} - SKIPPING (already exists)")
					continue
				...
				print(f"\n[{run_count}] IVF-PQ: N={N}, kclusters={param_set['kclusters']}, nprobe={param_set['nprobe']}, M={param_set['ivfpq_M']}")
